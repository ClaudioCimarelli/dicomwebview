...
def create_image_slice(plane, slice_n):
    ...
    # else:
    #     w = png.Writer(image_pixel.shape[1], image_pixel.shape[0], greyscale=True,
    #                    x_pixels_per_unit=10000 * image_pixel.shape[1] / pixelSpacing[1],
    #                    y_pixels_per_unit=10000 * image_pixel.shape[0] / pixelSpacing[0], unit_is_meter=True)

    w.write(png_file, image_pixel)
    png_file.seek(0)
    # img = Image.open(png_file)
    #
    # ratios = [pixelSpacing[0], pixelSpacing[1], pixelSpacing[2]]
    # #session['ratios'] = ratios
    # scaled_height = int(img.size[1]/ratios[plane])
    # img = img.resize((img.size[0], scaled_height), Image.ANTIALIAS)
    #
    # resized_png_file = StringIO.StringIO()
    # img.save(resized_png_file, format='PNG')
    # resized_png_file.seek(0)
